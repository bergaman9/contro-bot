...
logger = logging.getLogger('community.card_renderer')
...
async def create_ticket_card(guild, bot=None, max_members=100):
    ...
    try:
        ...
        if guild and bot:
            try:
                # Community cog ve ticket veritabanı
                cog = bot.get_cog("Community")
                if cog and hasattr(cog, "mongo_db"):
                    db = cog.mongo_db
                    ticket_coll = db.get("tickets")
        ...
        if bot and members:
            try:
                ...
                if cog and hasattr(cog, 'xp_manager'):
                    ...
                    try:
                        results = list(db.users.find({"user_id": {"$in": member_ids}}))
