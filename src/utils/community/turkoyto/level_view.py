...
logger = logging.getLogger('turkoyto.views.level')
...
class LevelLeaderboardView(View):
    ...
    async def create_leaderboard_embed(self):
        ...
        users = list(self.mongo_db.turkoyto_users.find(
            {"guild_id": self.guild_id}
        ).sort(self.sort_by, self.sort_direction).skip(skip).limit(self.items_per_page))
        
        # Get total count for pagination
        total_users = self.mongo_db.turkoyto_users.count_documents({"guild_id": self.guild_id})
...
class LevelRolesView(View):
    ...
    async def create_roles_embed(self):
        ...
        config = self.mongo_db.turkoyto_config.find_one({"guild_id": self.guild_id}) or {}
...
class AddLevelRoleModal(discord.ui.Modal, title="Seviye Rolü Ekle"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Update database
            self.mongo_db.turkoyto_config.update_one(
                {"guild_id": self.guild_id},
