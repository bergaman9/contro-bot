...
class UserService(BaseService):
    ...
    def __init__(self, db):
        ...
        super().__init__(db)
        self.user_collection = UserCollection(db)
    
    async def get_user(self, user_id: int) -> Optional[User]:
        """Get user by ID."""
        try:
            return await self.user_collection.find_by_user_id(user_id)
    ...
    async def ensure_user_exists(self, discord_user: discord.User) -> User:
        ...
        try:
            return await self.user_collection.get_or_create(
    ...
    async def add_xp(self, user_id: int, amount: int) -> Optional[tuple[int, int]]:
        ...
        try:
            result = await self.user_collection.add_global_xp(user_id, amount)
            if result:
    ...
    async def get_leaderboard(self, limit: int = 10) -> List[User]:
        """Get XP leaderboard."""
        try:
            return await self.user_collection.get_top_users_by_xp(limit)
        except Exception as e:
    ...
    async def add_badge(self, user_id: int, badge: str) -> bool:
        """Add a badge to user."""
        try:
            success = await self.user_collection.add_badge(user_id, badge)
            if success:
    ...
    async def remove_badge(self, user_id: int, badge: str) -> bool:
        ...
        try:
            success = await self.user_collection.remove_badge(user_id, badge)
            if success:
    ...
    async def get_users_by_badge(self, badge: str) -> List[User]:
        ...
        try:
            return await self.user_collection.get_users_by_badge(badge)
        except Exception as e:
